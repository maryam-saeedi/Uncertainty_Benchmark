...
@hydra.main(config_path="../configs", config_name="swag1", version_base=None)
def main(config: DictConfig) -> None:
    ...
    swag = Swag(config)
    swag.build_base_model(retrain=False, pretrained='results/common/model.pt')
    swag.build_method(rebuild=False)
    ...
    if os.path.exists(os.path.join(config.method.output.path, 'result', 'valid_uncertainties.pkl')):
        ...
        valid_uncertainties = swag.measure_uncertainty(val_dl, train_dl)
    ...
    for k, v in test_dl.items():
        if os.path.exists(os.path.join(config.method.output.path, 'result', f'test_uncertainties_{k}.pkl')):
            ...
            test_uncertainties = swag.measure_uncertainty(v, train_dl)
