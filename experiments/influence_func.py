...
from methods.influence_function import InfluenceFunction
...
@hydra.main(config_path="../configs", config_name="inf1", version_base=None)
def main(config: DictConfig) -> None:
    # Print the configuration
    print("Configuration loaded:")
    print(OmegaConf.to_yaml(config))

    inf_func = InfluenceFunction(config)
    inf_func.build_base_model(retrain=False, pretrained='results/common/model.pt')
    inf_func.build_method(rebuild=False)
    ...
    if os.path.exists(os.path.join(config.method.output.path, 'result', 'valid_uncertainties.pkl')):
        ...
        valid_uncertainties = inf_func.measure_uncertainty(val_dl)
    ...
    for k, v in test_dl.items():
        if os.path.exists(os.path.join(config.method.output.path, 'result', f'test_uncertainties_{k}.pkl')):
            ...
            test_uncertainties = inf_func.measure_uncertainty(v)
    ...
    for k_ in ['total_uncertainty', 'aleatoric_uncertainty', 'epistemic_uncertainty']:
        if isinstance(valid_uncertainties[k_], int):
            continue
        for k, v in test_dl.items():
