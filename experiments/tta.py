...
@hydra.main(config_path="../configs", config_name="tta1", version_base=None)
def main(config: DictConfig) -> None:
    ...
    tta = TTA(config)
    tta.build_base_model(retrain=False, pretrained='results/common/model.pt')
    ...
    if os.path.exists(os.path.join(config.method.output.path, 'result', 'valid_uncertainties.pkl')):
        ...
        valid_uncertainties = tta.measure_uncertainty(val_dl)
    ...
    for k, v in test_dl.items():
        if os.path.exists(os.path.join(config.method.output.path, 'result', f'test_uncertainties_{k}.pkl')):
            ...
            test_uncertainties = tta.measure_uncertainty(v)
